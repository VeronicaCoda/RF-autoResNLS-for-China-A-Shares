# RF-autoResNLS 代码实现说明

> **项目说明**：本项目包含 **RF-autoResNLS** 模型的 PyTorch 实现代码。这是一个混合深度学习模型，结合了 **ResNet**、**BiLSTM** 和 **Random Forest**的自适应权重机制，用于股票收盘价的时间序列预测。

## 🛠️ Requirements

本项目基于 Python 3 开发。运行代码前，请确保安装以下核心库：

  * **PyTorch**: 深度学习框架构建模型
  * **Baostock**: 获取中国股市历史数据
  * **Scikit-learn**: 数据预处理与随机森林算法
  * **Pandas & Numpy**: 数据处理
  * **Matplotlib**: 结果可视化

您可以运行以下命令安装依赖：

```bash
pip install torch pandas numpy scikit-learn matplotlib baostock
```

## 📂 Code Structure

Jupyter Notebook (`rf-autoresnls.ipynb`) 的执行流程如下：

### 1\. Data Preprocessing

  * **数据源**: 使用 `baostock` API 自动下载上证综指 (`sh.000001`) 的历史 K 线数据。
      * *时间范围*: 1992-01-01 至 2024-01-31。
      * *特征*: 仅使用收盘价 (`close`)。
  * **归一化**: 使用 `MinMaxScaler` 将数据缩放至 `[0, 1]` 区间。
  * **序列生成**: 采用滑动窗口法，时间步长 (Time Step) 设为 **5**。
      * `x`: 过去 5 个交易日的数据。
      * `y`: 第 6 个交易日的收盘价。

### 2\. Weight Initialization via Random Forest

在构建神经网络之前，代码首先训练一个 `RandomForestRegressor`：

  * **目的**: 计算过去 5 天（输入序列）对第 6 天价格的特征重要性 (Feature Importance)。
  * **作用**: 这些权重被归一化后，作为参数传递给 ResNLS 模型，用于在残差连接中自适应地调整 CNN 提取特征的权重。

### 3\. Model Architecture

模型类 `ResNLS` 定义在 PyTorch 中，主要包含两部分：

  * **CNN 残差模块**:
      * 包含两个 `Conv1d` 层（Kernel size=3）。
      * 配合 `ReLU` 激活函数、`BatchNorm1d` 和 `Dropout` (0.2)。
      * **创新点**: 这里的残差连接不是简单的相加，而是乘以了步骤 2 中计算出的 `weight` (自适应权重)。
  * **BiLSTM 层**:
      * 双向 LSTM (`bidirectional=True`)，隐藏层维度为 32。
      * 用于捕捉时间序列的长短期依赖。
  * **全连接层**: 输出最终预测值。

### 4\. Training

  * **优化器**: Adam (Learning rate = 1e-3, Weight decay = 1e-5)。
  * **损失函数**: MSELoss (均方误差)。
  * **学习率调度**: `ReduceLROnPlateau` (当 Loss 不再下降时自动降低学习率)。
  * **参数**: Epochs = 50, Batch Size = 64。

### 5\. Validation & Visualization

  * 模型在测试集上进行评估。
  * 输出评估指标：**MAE**, **MSE**, **RMSE**。
  * 绘制损失函数曲线 (Train vs Validation Loss) 和 股价预测对比图 (Actual vs Predicted)。

## 🚀 Quick Start

1.  克隆本仓库或下载 `rf-autoresnls.ipynb`。
2.  确保已安装上述依赖库。
3.  直接运行 Jupyter Notebook 的所有单元格。
      * *注意：首次运行需要联网下载 Baostock 数据。*

## 📊 Results



### 预测效果对比

模型在测试集上的拟合情况：

> <img width="1334" height="709" alt="image" src="https://github.com/user-attachments/assets/3b70f005-6352-45a3-ab5a-b446c8c7a649" />


### 性能指标

在测试集上的最终评估结果：

  * **MAE**: \~29.26
  * **MSE**: \~1430.58
  * **RMSE**: \~37.82
    *(注：具体数值可能因随机种子或数据更新略有不同)*

## 📝 核心代码片段

**Random Forest:**

```python
rf = RandomForestRegressor(n_estimators=100, random_state=42)
rf.fit(x_train_5.reshape(-1,5), y_train_5)
weight = rf.feature_importances_
# 权重归一化处理...
```

**ResNLS Forwarding:**

```python
def forward(self, x):
    cnn_output = self.cnn(x)
    cnn_output = cnn_output.view(-1, 1, n_input)
    x = x.view(-1, 1, n_input)
    
    # 关键步骤：加入自适应权重的残差连接
    residuals = x + cnn_output * self.weight 
    
    _, (h_n, _)  = self.lstm(residuals)
    y_hat = self.linear(h_n[0,:,:])
    return y_hat
```

-----

*基于 "区块链+金融创新" 期末作业代码整理。*
